...
1.  **克隆项目** (如果从源码部署):
    ```bash
    # 替换为您的实际仓库地址
    git clone [https://github.com/imsyy/UNM-Server-AXC.git](https://github.com/imsyy/UNM-Server-AXC.git)
    cd UNM-Server-AXC
    ```
...
**示例请求:**
```http
GET [https://api-unms.axingchen.com/](https://api-unms.axingchen.com/)
...
参数: 无
示例请求:
GET [https://api-unms.axingchen.com/info](https://api-unms.axingchen.com/info)
...
参数: 无
示例请求:
GET [https://api-unms.axingchen.com/test](https://api-unms.axingchen.com/test)
...
成功响应示例 (200 OK):
{
  ...
  "data": {
    "url": "[http://example.com/song.mp3](http://example.com/song.mp3)", // 实际播放链接
...
示例请求:
GET [https://api-unms.axingchen.com/match?id=1962165898&server=kuwo,qq](https://api-unms.axingchen.com/match?id=1962165898&server=kuwo,qq)
...
成功响应示例 (200 OK):
{
  ...
  "data": {
    "url": "[http://example.com/matched_song.mp3](http://example.com/matched_song.mp3)",
    "br": 320000,
    "size": 10240000,
    "type": "flac",
    "md5": "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx",
    "source": "kuwo", // 成功匹配的音源
    "proxyUrl": "[http://yourproxy.com/http/example.com/matched_song.mp3](http://yourproxy.com/http/example.com/matched_song.mp3)" // 如果配置了代理且链接满足代理条件
  }
}
...
示例请求:
GET [https://api-unms.axingchen.com/url?id=歌曲TrackID&source=netease&br=320](https://api-unms.axingchen.com/url?id=歌曲TrackID&source=netease&br=320)
...
成功响应示例 (200 OK):
{
  ...
  "data": {
    ...
    "br": 320000, // 实际获取到的比特率
    "size": 10485760, // 文件大小
    "url": "[http://example.com/song_from_netease.mp3](http://example.com/song_from_netease.mp3)", // 播放链接
    "source": "netease", // 实际使用的音源
    "proxyUrl": "[http://yourproxy.com/http/example.com/song_from_netease.mp3](http://yourproxy.com/http/example.com/song_from_netease.mp3)" // 如果适用
  }
}
...
示例请求:
GET [https://api-unms.axingchen.com/search?name=七里香&source=kugou&count=5](https://api-unms.axingchen.com/search?name=七里香&source=kugou&count=5)
...
示例请求:
GET [https://api-unms.axingchen.com/pic?id=图片ID1_kugou&source=kugou&size=500](https://api-unms.axingchen.com/pic?id=图片ID1_kugou&source=kugou&size=500)
...
成功响应示例 (200 OK):
{
  ...
  "message": "请求成功 (源: kugou)",
  "data": {
    "url": "[http://example.com/album_cover.jpg](http://example.com/album_cover.jpg)", // 封面图片链接
    "source": "kugou" // 实际使用的音源
  }
}
...
| source | string | 否 | API_CONFIG.defaultSource | 指定音源。 |
示例请求:
GET [https://api-unms.axingchen.com/lyric?id=歌词ID1_kugou&source=kugou](https://api-unms.axingchen.com/lyric?id=歌词ID1_kugou&source=kugou)
...
主要环境变量：
 * PORT: 服务监听端口 (例如 5678，本地开发时有效)。
 * NODE_ENV: Node.js 运行环境 (production 或 development)。影响日志级别和JSON美化。
 * ALLOWED_DOMAIN: CORS 设置，允许访问 API 的前端域名 (例如 https://your-frontend.com，或 * 允许所有，但不推荐用于生产)。
 * PROXY_URL: （可选）反向代理服务器的 URL，用于代理某些音源的 HTTP 链接以解决 HTTPS 页面的混合内容问题 (例如 https://your-proxy.com/proxy?url=)。
 * CACHE_ENABLED: 是否启用缓存 (true 或 false)。默认为 true。
 * CACHE_DEFAULT_TTL_SECONDS: 缓存默认的生存时间（秒）。默认为 3600 (1小时)。
 * ENABLE_FLAC, SELECT_MAX_BR, FOLLOW_SOURCE_ORDER: 用于 @unblockneteasemusic/server 库的功能开关。
 * NETEASE_COOKIE, QQ_COOKIE, MIGU_COOKIE, JOOX_COOKIE, YOUTUBE_KEY: 各平台 Cookie 或 API Key，用于解锁VIP歌曲或解决访问限制。按需填写。
...
注：默认启用 列的 ✅ 表示该音源包含在代码的 UNM_SETTINGS.defaultSources 数组中。未标记的音源仍可通过 /match 接口的 server 参数指定使用。
🔗 关于反向代理
 ...
 * [ ] 更细致的缓存策略: 例如为不同类型的API或不同音源配置不同的缓存TTL。
 * [ ] Docker 化: 提供 Dockerfile 以方便容器化部署。
 * [ ] 请求频率限制: 实现基础的速率限制以保护服务。
🤝 贡献
